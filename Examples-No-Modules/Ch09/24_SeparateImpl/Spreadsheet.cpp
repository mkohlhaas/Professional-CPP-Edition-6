...
Spreadsheet::Impl::Impl (size_t width, size_t height)
    : m_id{ ms_counter++ }, m_width{ min (width, Spreadsheet::MaxWidth) },
...
void
Spreadsheet::Impl::verifyCoordinate (size_t x, size_t y) const
{
  if (x >= m_width)
    {
...
void
Spreadsheet::Impl::setCellAt (size_t x, size_t y, const SpreadsheetCell &cell)
{
...
Spreadsheet::Impl::getCellAt (size_t x, size_t y) const
{
  verifyCoordinate (x, y);
  return m_cells[x][y];
}
...
SpreadsheetCell &
Spreadsheet::Impl::getCellAt (size_t x, size_t y)
{
  return const_cast<SpreadsheetCell &> (as_const (*this).getCellAt (x, y));
}

void
Spreadsheet::Impl::swap (Impl &other) noexcept
{
...
Spreadsheet::Impl &
Spreadsheet::Impl::operator= (const Impl &rhs)
{
  // Copy-and-swap idiom
