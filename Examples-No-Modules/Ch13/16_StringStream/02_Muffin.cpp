...
template <> class std::formatter<Muffin>
{
public:
  constexpr auto
  parse (auto &context)
  ...
  auto
  format (const Muffin &muffin, auto &ctx) const
  {
    ctx.advance_to (format_to (ctx.out (), "{}, size is {}, {}", muffin.getDescription (), muffin.getSize (),
...
Muffin
createMuffin (istringstream &stream)
{
  Muffin muffin;
