...
template <typename T> optional<size_t> Find(const T &value, const T *arr, size_t size)
{
...
template <typename T, size_t N> optional<size_t> Find(const T &value, const T (&arr)[N])
{
    return Find(value, arr, N);
}

template <std::floating_point T> bool AreEqual(T x, T y, int precision = 2)
{
...
template <std::floating_point T> optional<size_t> Find(const T &value, const T *arr, size_t size)
{
