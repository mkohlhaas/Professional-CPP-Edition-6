...
template <typename T, typename Allocator = std::allocator<T>> class ObjectPool final
{
  public:
    ObjectPool() = default;
    ...
    template <typename... Args> std::shared_ptr<T> acquireObject(Args &&...args);
...
ObjectPool<T, Allocator>::ObjectPool(const Allocator &allocator) : m_allocator{allocator}
{
}
