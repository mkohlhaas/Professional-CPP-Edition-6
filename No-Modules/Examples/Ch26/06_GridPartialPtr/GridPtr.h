...
template <typename U> class Grid<U *>
{
  public:
...
template <typename U> Grid<U *>::Grid(std::size_t width, std::size_t height) : m_width{width}, m_height{height}
{
    m_cells.resize(m_width * m_height);
}

template <typename U> void Grid<U *>::swap(Grid &other) noexcept
{
    using std::swap;
...
template <typename U> Grid<U *>::Grid(const Grid &src) : Grid{src.m_width, src.m_height}
{
...
template <typename U> Grid<U *> &Grid<U *>::operator=(const Grid &rhs)
{
...
template <typename U> Grid<U *>::Grid(Grid &&src) noexcept
{
    swap(src);
}

template <typename U> Grid<U *> &Grid<U *>::operator=(Grid &&rhs) noexcept
{
...
template <typename U> void Grid<U *>::verifyCoordinate(std::size_t x, std::size_t y) const
{
    if (x >= m_width)
    {
...
template <typename U> const std::unique_ptr<U> &Grid<U *>::at(std::size_t x, std::size_t y) const
{
    verifyCoordinate(x, y);
    return m_cells[x + y * m_width];
}

template <typename U> std::unique_ptr<U> &Grid<U *>::at(std::size_t x, std::size_t y)
{
