...
template <typename T> bool directed_graph<T>::insert(T node_value)
{
...
template <typename T> bool directed_graph<T>::erase(const T &node_value)
{
    auto iter{find_node(node_value)};
    if (iter == std::end(m_nodes))
    {
...
template <typename T> const T &directed_graph<T>::operator[](std::size_t index) const
{
...
template <typename T> std::set<T> directed_graph<T>::get_adjacent_nodes_values(const T &node_value) const
{
    auto iter{find_node(node_value)};
    if (iter == std::end(m_nodes))
    {
...
template <typename T> std::size_t directed_graph<T>::size() const noexcept
{
    return m_nodes.size();
}
} // namespace ProCpp
