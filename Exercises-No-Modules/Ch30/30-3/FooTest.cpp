...
void
FooTest::testConstructorALessThanB ()
{
  // IF nothing
...
void
FooTest::testConstructorABiggerThanB ()
{
  // IF nothing

  // WHEN constructing a Foo with a > b
  // THEN an invalid_argument exception is thrown.
  Assert::ExpectException<std::invalid_argument> ([] { Foo f{ 2, 1 }; });
}

void
FooTest::testConstructorAEqualB ()
{
  // IF nothing

  // WHEN constructing a Foo with a == b
  // THEN an invalid_argument exception is thrown.
  Assert::ExpectException<std::invalid_argument> ([] { Foo f{ 2, 2 }; });
}
