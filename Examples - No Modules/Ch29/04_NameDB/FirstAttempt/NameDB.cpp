...
bool
NameDB::nameExists (const string &name) const
{
...
void
NameDB::incrementNameCount (const string &name)
{
  for (auto &entry : m_names)
    {
      if (entry.first == name)
...
void
NameDB::addNewName (const string &name)
{
...
int
NameDB::getNameRank (const string &name) const
{
...
int
NameDB::getAbsoluteNumber (const string &name) const
{
  for (auto &entry : m_names)
    {
      if (entry.first == name)
