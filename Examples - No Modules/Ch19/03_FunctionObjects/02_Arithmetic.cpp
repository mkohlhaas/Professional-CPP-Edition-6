...
auto
accumulateData (Iter begin, Iter end, const StartValue &startValue, Operation op)
{
...
double
geometricMean (span<const int> values)
{
...
double
geometricMeanTransparent (span<const int> values)
{
...
main ()
{
  ...
  println ("The geometric mean is (transparent function object) {}", geometricMeanTransparent (myVector));
  ...
  {
    vector<int> values{ 1, 2, 3 };
    double      result{ accumulateData (cbegin (values), cend (values), 1.1, multiplies<int>{}) };
    println ("{}", result);
  }
}
