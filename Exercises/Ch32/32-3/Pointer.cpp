...
public:
  ...
  template <convertible_to<T> E> void assign (const E &value);

private:
  T *m_data{ nullptr };
};

template <typename T> Pointer<T>::Pointer (T *data) : m_data{ data }
{
  if (data == nullptr)
    {
...
template <typename T> Pointer<T>::~Pointer () { reset (); }

template <typename T> Pointer<T>::Pointer (Pointer &&src) noexcept : m_data{ std::exchange (src.m_data, nullptr) } {}

template <typename T>
Pointer<T> &
Pointer<T>::operator= (Pointer &&rhs) noexcept
{
  if (this != &rhs)
    {
      reset ();
...
template <typename T>
void
Pointer<T>::reset (T *data /*= nullptr*/) noexcept
{
  if (m_data)
    {
      delete m_data;
...
void
Pointer<T>::assign (const E &value)
{
  if (m_data)
    {
      *m_data = value;
