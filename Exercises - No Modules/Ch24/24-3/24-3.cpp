...
Parameter
parseParameter (const char *parameter)
{
  regex r{ "(.*)=(.*)" };
      ...
      try
        {
          ...
          if (holds_alternative<bool> (parameter.second))
            {
              println ("Boolean: {} -> {}", parameter.first, get<bool> (parameter.second));
            }
          else if (holds_alternative<int> (parameter.second))
            {
              println ("Integer: {} -> {}", parameter.first, get<int> (parameter.second));
            }
          else if (holds_alternative<string> (parameter.second))
            {
              println ("String: {} -> {}", parameter.first, get<string> (parameter.second));
            }
        }
