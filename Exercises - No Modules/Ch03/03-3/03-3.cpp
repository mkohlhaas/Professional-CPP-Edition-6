...
public:
  ...
  void setSourceType (
      std::string_view type); //// _Type: identifiers should not start with an underscore followed by a capital.

private:
  double
      m_current; // The X-ray current in µA.              //// d: too short and meaningless. Data members start with m_
  std::string m_sourceType; // The type of the X-ray source. //// m_src__type: double underscores and abbreviations are
                            // not allowed.
};
